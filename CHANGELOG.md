...
- Hardened the DocumentsProvider contract with dynamic root discovery, persisted-grant revocation handling, removable-root reappearance, least-privilege mutation flags, metadata coverage, and symlink-safe document IDs.
...
- [ ] P1: Add migration, adapter, archive, and DocumentsProvider regression coverage
  ...
  - Evidence: `core/database` schema 2 to 7 and `AppDatabase.kt`; existing 32 test files; `FileDocumentsProvider`; `ArchiveEntryPathPolicy`; Android DocumentsProvider contract; `RESEARCH.md` testability assessment.
...
- [ ] P2: Add release smoke coverage for exported surfaces and large screens
  - Why: FileExplorer has many exported or system-facing paths, including DocumentsProvider, tile, FileProvider, share intents, package queries, notifications, Cast, and a broad Compose layout surface.
